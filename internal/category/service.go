...
func toProtoTarget(s string) apiv1.CategoryTargetType {
	switch s {
	case "worker":
		return apiv1.CategoryTargetType_CATEGORY_TARGET_TYPE_WORKER
	case "workflow":
		return apiv1.CategoryTargetType_CATEGORY_TARGET_TYPE_WORKFLOW
	case "conversation":
		return apiv1.CategoryTargetType_CATEGORY_TARGET_TYPE_CONVERSATION
	default:
...
func (s *Service) ListCategories(ctx context.Context, req *connect.Request[apiv1.ListCategoriesRequest]) (*connect.Response[apiv1.ListCategoriesResponse], error) {
	...
	cats, err := db.ListCategories(ctx, tx.Tx, tenantID, tt)
	if err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}
	assigns, err := db.ListAssignments(ctx, tx.Tx, tenantID, tt)
	if err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}
	_ = tx.Commit(ctx)
	...
	for _, c := range cats {
		pc = append(pc, toProto(c))
	}
	var pa []*apiv1.CategoryAssignment
	for _, a := range assigns {
		pa = append(pa, &apiv1.CategoryAssignment{EntityId: a.EntityID, CategoryId: a.CategoryID, TargetType: toProtoTarget(a.TargetType)})
	}
...
func (s *Service) CreateCategory(ctx context.Context, req *connect.Request[apiv1.CreateCategoryRequest]) (*connect.Response[apiv1.CreateCategoryResponse], error) {
	...
	if err := recordAudit(ctx, tx.Tx, tenantID, "category.created", "category", cat.ID, nil, audit.Snapshot(cat)); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}
...
func (s *Service) UpdateCategory(ctx context.Context, req *connect.Request[apiv1.UpdateCategoryRequest]) (*connect.Response[apiv1.UpdateCategoryResponse], error) {
	...
	defer tx.Rollback(ctx)
	before, err := db.GetCategory(ctx, tx.Tx, tenantID, req.Msg.Id)
	if err != nil {
	...
	if err := recordAudit(ctx, tx.Tx, tenantID, "category.updated", "category", updated.ID, audit.Snapshot(before), audit.Snapshot(updated)); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}
	if err := tx.Commit(ctx); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
...
func (s *Service) DeleteCategory(ctx context.Context, req *connect.Request[apiv1.DeleteCategoryRequest]) (*connect.Response[apiv1.DeleteCategoryResponse], error) {
	...
	if err := db.DeleteCategory(ctx, tx.Tx, tenantID, req.Msg.Id); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}
	if err := recordAudit(ctx, tx.Tx, tenantID, "category.deleted", "category", before.ID, audit.Snapshot(before), nil); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}
	if err := tx.Commit(ctx); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
...
func (s *Service) AssignToCategory(ctx context.Context, req *connect.Request[apiv1.AssignToCategoryRequest]) (*connect.Response[apiv1.AssignToCategoryResponse], error) {
	...
	if req.Msg.EntityId == "" || req.Msg.CategoryId == "" {
		return nil, connect.NewError(connect.CodeInvalidArgument, nil)
	}
	tx, err := s.pool.BeginTenantTx(ctx, tenantID)
	if err != nil {
	...
	if err := recordAudit(ctx, tx.Tx, tenantID, "category.assigned", "category", req.Msg.CategoryId, nil, audit.Snapshot(map[string]string{"entity_id": req.Msg.EntityId, "target_type": tt})); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}
	if err := tx.Commit(ctx); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
...
func (s *Service) UnassignFromCategory(ctx context.Context, req *connect.Request[apiv1.UnassignFromCategoryRequest]) (*connect.Response[apiv1.UnassignFromCategoryResponse], error) {
	...
	if err := recordAudit(ctx, tx.Tx, tenantID, "category.unassigned", "category", req.Msg.EntityId, nil, nil); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}
	if err := tx.Commit(ctx); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
...
func (s *Service) ReorderCategories(ctx context.Context, req *connect.Request[apiv1.ReorderCategoriesRequest]) (*connect.Response[apiv1.ReorderCategoriesResponse], error) {
	...
	if err := recordAudit(ctx, tx.Tx, tenantID, "category.reordered", "category", tenantID, audit.Snapshot(before), audit.Snapshot(after)); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
	}
	if err := tx.Commit(ctx); err != nil {
		return nil, connect.NewError(connect.CodeInternal, err)
