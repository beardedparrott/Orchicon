...
func helpLines(routes []KeyRoute) []string {
	...
	out = append(out, theme.HintText.Render("Tabs: enter/space on the active tab opens its submenu of sub-screens (from content focus, or from an empty composer) — up/down (or j/k) move, enter selects, esc closes; the rows are mouse click targets. ctrl+o/w/e/a/f/t switch areas, click a tab to switch + open its menu."))
