...
func helpLines(routes []KeyRoute) []string {
	...
	out = append(out, theme.HintText.Render("Attachments: ctrl+v attaches an IMAGE FROM THE CLIPBOARD (a screenshot has no path, so it cannot be typed in), and ctrl+f attaches a FILE BY PATH. Both land in the pending set for the NEXT message and are reported in the strip above the composer; the model receives an image as vision input and a text file inline, which is the same contract the GUI's attachment control uses. The chords are also on the help list here rather than in the composer's affordance row, because that row is ONE LINE BY CONTRACT — adding two chords to it wrapped it and pushed it out of the box."))
	out = append(out, theme.HintText.Render("Chat dock: the composer is focused at launch — type immediately; ctrl+g/esc toggle content↔composer. / opens the command palette (above the composer, input stays visible). Enter sends; alt+enter (or a trailing \\ then Enter) inserts a newline; pasted text never sends until Enter. ctrl+y STOPS the in-flight reply (the GUI's Stop button) — the affordance row offers it while a reply streams, and sending while a reply streams interjects (supersedes the turn) instead. /help lists slash commands."))
