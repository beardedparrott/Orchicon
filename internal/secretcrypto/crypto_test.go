...
func TestEncryptDecryptRoundtrip(t *testing.T) {
	...
	if string(got) != pt {
		t.Fatalf("roundtrip got %q want %q", string(got), pt)
	}
	// ciphertext is base64 and varies per nonce
	ct2, _ := Encrypt([]byte(pt), kek)
	if ct == ct2 {
		t.Log("warning: same nonce (unlikely)")
	}
...
func TestEncryptWrongKEKSize(t *testing.T) {
	...
	if _, err := Decrypt("abc", make([]byte, 16)); err == nil {
		t.Fatal("expected error for short kek")
	}
}
...
func TestDecryptWrongKEK(t *testing.T) {
	...
	if _, err := Decrypt(ct, k2); err == nil {
		t.Fatal("expected decrypt failure with wrong kek")
	}
}
...
func TestParseKEK(t *testing.T) {
	...
	k2, err := ParseKEK(b64)
	if err != nil {
		t.Fatalf("b64 kek: %v", err)
	}
	if string(k2) != raw {
		t.Fatal("b64 mismatch")
	}
	if _, err := ParseKEK("short"); err == nil {
...
func TestLoadOrCreateKEK(t *testing.T) {
	...
	if string(k1) != string(k2) {
		t.Fatal("KEK changed across loads — stored secrets would be lost")
	}
...
func TestLoadOrCreateKEKRejectsCorruptFile(t *testing.T) {
	...
	if err := os.WriteFile(KEKPath(dir), []byte("not-a-32-byte-key"), 0o600); err != nil {
		t.Fatal(err)
	}
...
func TestResolveKEK(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("resolve file again: %v", err)
	}
	if string(k1) != string(k3) {
		t.Fatal("instance KEK changed")
	}
}
