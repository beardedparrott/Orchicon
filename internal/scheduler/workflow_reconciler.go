...
func (r *WorkflowReconciler) buildCompositePrompt(ctx context.Context, tx pgx.Tx, tenantID string, wi db.WorkItemRow, worker db.WorkerVersionRow, allSteps []workflow.StepWire, runs map[string]db.WorkflowStepRunRow) (string, error) {
	...
	// Cross-fire facts forward (PR B — context propagation): on a
	// recurring work item the AcceptanceReview is overwritten with each
	// fire's acceptance review (buildAcceptanceReview), whose per-step
	// summaries carry the worker-emitted FACTS LEARNED lines. Forward
	// them so this fire INHERITS the previous fire's corrections
	// (source quirks, dedupe states, tooling gotchas) instead of
	// re-deriving them. Renders "" when there is nothing to carry, so
	// non-recurring items are byte-identical to before.
	sb.WriteString(carriedFactsBlock(wi.AcceptanceReview))
...
// carriedFactsMaxChars caps the cross-fire facts-forward block. The
// acceptance review's total budget is maxDescLen (64K), but the valuable
// payload — FACTS LEARNED lines plus the delivery summary — is a small
// fraction of it; 8K chars (~2k tokens) is ample headroom while keeping
// the carry-forward a rounding error against the rest of the prompt.
const carriedFactsMaxChars = 8 * 1024

// carriedFactsBlock renders the cross-fire "carry-forward" section for a
// recurring work item's next fire, from the PREVIOUS fire's acceptance
// review still stored on the item (the reconciler overwrites
// AcceptanceReview on every fire, so at fire time it holds fire N-1).
//
// The acceptance review embeds each step's run summary verbatim, and
// workers are contracted to emit `FACTS LEARNED:` lines there — the
// established within-run facts channel. This block crosses the same
// ledger ACROSS fires so fire N+1 inherits fire N's corrections
// (source quirks, dedupe states, tooling gotchas) instead of paying to
// re-derive them. Pure function of the review string: no DB calls, and
// an empty/whitespace review renders "" (first fire, manual runs, and
// non-recurring items are byte-identical to not calling this at all).
//
// Extraction rules, in order:
//  1. If the review contains any non-instructional FACTS LEARNED lines,
//     forward those lines only (plus a tail-capped delivery summary for
//     grounding). "Instructional" lines (the worker-facing contract
//     sentences that themselves start with "FACTS LEARNED:" inside the
//     composite's facts-ledger directive) are skipped so the block
//     never carries the boilerplate contract back into a prompt.
//  2. Otherwise, forward the review's tail (bounded), which always
//     includes the fire's delivery summary — still context, just
//     without the per-fact pinning.
// The rendered block is itself capped at carriedFactsMaxChars.
func carriedFactsBlock(prevReview string) string {
	prevReview = strings.TrimSpace(prevReview)
	if prevReview == "" {
		return ""
	}

	// Extract FACTS LEARNED lines, skipping the instructional examples
	// that begin the facts-ledger directive (they quote the prefix
	// inside a sentence or wrap it in backticks). Both the plain
	// "FACTS LEARNED:" lines workers emit and the "FACTS LEARNED (from
	// <step>):" form the TaskReconciler writes to the .orchicon ledger
	// are accepted.
	var facts []string
	for _, line := range strings.Split(prevReview, "\n") {
		t := strings.TrimSpace(line)
		if t == "" {
			continue
		}
		if !strings.HasPrefix(t, "FACTS LEARNED") || !strings.Contains(t, ":") {
			continue
		}
		if t != "FACTS LEARNED:" && !isInstructionalFactLine(t) {
			facts = append(facts, t)
		}
	}

	var sb strings.Builder
	sb.WriteString("## Carry-forward — facts learned by the previous fire\n\n")
	sb.WriteString("This is a recurring work item. The block below is extracted from the previous fire's acceptance review so you inherit what it established instead of re-deriving it. Do not re-verify these facts; if one proves wrong, append a correcting `FACTS LEARNED:` line.\n\n")
	...
	if len(facts) > 0 {
		...
		// Ground the facts with the fire's delivery summary: the last
		// "### What was delivered" section of the acceptance review,
		// capped to whatever budget the facts left. stripFactLines
		// removes the fact lines (already carried above) and any
		// contract boilerplate — the delivery section re-embeds step
		// summaries verbatim, so without it each fact would ride twice
		// and the composite's own instruction could echo back in.
		if idx := strings.LastIndex(prevReview, "### What was delivered"); idx >= 0 {
			tail := stripFactLines(strings.TrimSpace(prevReview[idx:]))
			if tail != "" {
				if budget := carriedFactsMaxChars - sb.Len() - 128; budget > 0 && len(tail) > budget {
					tail = tail[:budget] + "\n…[truncated]"
				}
				fmt.Fprintf(&sb, "\nPrevious fire's delivery summary (tail, capped):\n%s\n", tail)
			}
		}
	} else {
		// No extractable facts — still forward the review's tail so the
		// fire starts from the previous delivery context (contract
		// boilerplate filtered out; degenerate all-facts reviews fall
		// back to the raw tail so the block never renders empty).
		tail := stripFactLines(prevReview)
		if tail == "" {
			tail = prevReview
		}
		if len(tail) > carriedFactsMaxChars {
			tail = tail[len(tail)-carriedFactsMaxChars:]
		}
		fmt.Fprintf(&sb, "Previous fire's acceptance review (tail, capped):\n%s\n", tail)
	}

	out := sb.String()
	if len(out) > carriedFactsMaxChars {
		out = out[:carriedFactsMaxChars]
	}
	return out + "\n"
}

// isInstructionalFactLine reports whether a stripped line is the
// facts-ledger contract boilerplate (the composite's own instruction on
// how to emit facts — "…record it as a `FACTS LEARNED:` line…") rather
// than a fact. Instructional lines must never be carried into a prompt:
// the prompt already teaches the contract, and re-teaching it inside a
// facts block teaches the model to echo instructions as facts.
func isInstructionalFactLine(t string) bool {
	return strings.Contains(t, "record it as") && strings.Contains(t, "FACTS LEARNED:")
}

// stripFactLines removes FACTS LEARNED-prefixed lines (either the plain
// or the "FACTS LEARNED (from <step>):" form) and instructional contract
// lines from a block, keeping everything else (delivery bullets, prose).
// Used to dedupe the delivery-summary tail against the extracted fact
// list and to scrub the fallback tail.
func stripFactLines(s string) string {
	var kept []string
	for _, line := range strings.Split(s, "\n") {
		lt := strings.TrimSpace(line)
		if strings.HasPrefix(lt, "FACTS LEARNED") || isInstructionalFactLine(lt) {
			continue
		}
...
func (r *WorkflowReconciler) pollLoopDecisionChain(ctx context.Context, tx pgx.Tx, tenantID string, sr db.WorkflowStepRunRow, allSteps []workflow.StepWire) (bool, error) {
	...
	for _, cid := range chain {
		...
		// A chain step is complete once it reaches a terminal state.
		// Only SUCCEEDED unblocks (a failed reviewer triggers recovery
		// via the anyFailed path).
		if active.Status != domain.StepRunSucceeded {
			return false, nil
		}
	}
	return true, nil
