...
func TestValidateTargetType(t *testing.T) {
	for _, good := range []string{"worker", "workflow", "conversation"} {
		if err := ValidateTargetType(good); err != nil {
			t.Fatalf("valid %q rejected: %v", good, err)
		}
	}
	for _, bad := range []string{"", "WORKER", "workers", "projects", "conversation "} {
		if err := ValidateTargetType(bad); err == nil {
			t.Fatalf("invalid %q should fail", bad)
		}
	}
}
...
func TestCategoryTargetTypeIsolation_Unit(t *testing.T) {
	...
	if len(validTargetTypes) != 3 {
		t.Fatalf("expected 3 target types, got %d", len(validTargetTypes))
	}
	...
	for _, tt := range []string{"worker", "workflow", "conversation"} {
		if err := ValidateTargetType(tt); err != nil {
			t.Fatalf("%q should validate", tt)
		}
	}
}
