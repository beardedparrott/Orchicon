...
func UpdateTenantSettings(ctx context.Context, tx pgx.Tx, tenantID string, in TenantSettingsRow) (TenantSettingsRow, error) {
	const q = `INSERT INTO tenant_settings (
		...
		ON CONFLICT (tenant_id) DO UPDATE SET
			...
			backup_schedule = CASE WHEN $9 <> '' THEN $9 ELSE tenant_settings.backup_schedule END,
			backup_retention_days = CASE WHEN $10 <> 0 THEN $10 ELSE tenant_settings.backup_retention_days END,
			backup_directory = CASE WHEN $11 <> '' THEN $11 ELSE tenant_settings.backup_directory END,
			updated_at = now()
