...
func toolListCategories(ctx context.Context, pool *db.Pool, args json.RawMessage) (json.RawMessage, error) {
	...
	if err := db.ValidateTargetType(in.TargetType); err != nil {
		return nil, err
	}
	...
	cats, err := db.ListCategories(ctx, tx.Tx, tenantID, in.TargetType)
	if err != nil {
		return nil, err
	}
	assigns, err := db.ListAssignments(ctx, tx.Tx, tenantID, in.TargetType)
	if err != nil {
		return nil, err
	}
	_ = tx.Commit(ctx)
...
func toolCreateCategory(ctx context.Context, pool *db.Pool, args json.RawMessage) (json.RawMessage, error) {
	...
	defer tx.Rollback(ctx)
	cat, err := db.CreateCategory(ctx, tx.Tx, tenantID, in.TargetType, in.Name, in.Description)
	if err != nil {
		return nil, err
	}
...
func toolUpdateCategory(ctx context.Context, pool *db.Pool, args json.RawMessage) (json.RawMessage, error) {
	...
	if np == nil && dp == nil {
		return nil, fmt.Errorf("name or description required")
	}
	cat, err := db.UpdateCategory(ctx, tx.Tx, tenantID, in.ID, np, dp)
	if err != nil {
		return nil, err
	}
	if err := tx.Commit(ctx); err != nil {
...
func toolDeleteCategory(ctx context.Context, pool *db.Pool, args json.RawMessage) (json.RawMessage, error) {
	...
	defer tx.Rollback(ctx)
	if err := db.DeleteCategory(ctx, tx.Tx, tenantID, in.ID); err != nil {
		return nil, err
	}
	if err := tx.Commit(ctx); err != nil {
		return nil, err
...
func toolAssignToCategory(ctx context.Context, pool *db.Pool, args json.RawMessage) (json.RawMessage, error) {
	...
	defer tx.Rollback(ctx)
	if err := db.AssignToCategory(ctx, tx.Tx, tenantID, in.TargetType, in.EntityID, in.CategoryID); err != nil {
		return nil, err
	}
	if err := tx.Commit(ctx); err != nil {
		return nil, err
...
func toolUnassignFromCategory(ctx context.Context, pool *db.Pool, args json.RawMessage) (json.RawMessage, error) {
	...
	defer tx.Rollback(ctx)
	if err := db.UnassignFromCategory(ctx, tx.Tx, tenantID, in.TargetType, in.EntityID); err != nil {
		return nil, err
	}
	if err := tx.Commit(ctx); err != nil {
		return nil, err
...
func toolReorderCategories(ctx context.Context, pool *db.Pool, args json.RawMessage) (json.RawMessage, error) {
	...
	defer tx.Rollback(ctx)
	if err := db.ReorderCategories(ctx, tx.Tx, tenantID, in.TargetType, in.OrderedIDs); err != nil {
		return nil, err
	}
	if err := tx.Commit(ctx); err != nil {
		return nil, err
