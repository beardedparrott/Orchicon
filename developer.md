...
- Runtime adapters: pluggable gRPC sidecars, with a **built-in native engine as the default** — sessions run inside the control plane, so **no adapter CLI is required**. External adapters (OpenCode, and future ones) are optional and mounted, never baked: the operator installs one on the host, and `container.sh` / `orchicon install` / the runtime daemon bind-mount `~/.opencode` into the main and runtime containers **only when a run's boot profile demands it**. Session transport is the adapter contract: an OpenCode-worker execution runs as a persistent opencode session (`internal/opencode/session.go`), not a one-shot `run` subprocess.
