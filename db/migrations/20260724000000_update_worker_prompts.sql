...
-- 4. Ensure the newly published versions have the right status (idempotent).
UPDATE worker_versions SET status = 'published' WHERE id IN ('wv_se_pr_reviewer_v4', 'wv_se_qa_engineer_v3');

-- 5. Set purpose on all pre-canned workers so the composite prompt's
