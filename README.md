...
**v0.1.142** — HF models script optimization.
Optimized `scripts/hf-latest-models.sh`: replaced the per-model `jq` subprocess loop (5+ calls per model) with a single `jq` invocation, making `--week` mode (3000 models) complete in under a second instead of timing out.
