...
| Feature | **Wall-clock timeout is now a tenant setting.** The hard per-execution deadline (and its **3600s default** when a worker's `budget_overrides` are empty `{}`) is now configurable under **Settings → Defaults → Recovery stall parameters → Wall clock timeout (seconds)** — the same place as the other stall settings — with a new `ORCHICON_STALL_WALL_CLOCK_SECONDS` env override. Precedence: worker's explicit `budget_overrides.wall_clock_seconds` → tenant setting → env → built-in default (3600); an explicit worker `0` still disables that worker's timeout. This closes the gap where an empty `{}` budget left a long-running worker with **no hard stop at all** (the PR Reviewer ran 48+ min unchecked). New `tenant_settings.stall_wall_clock_seconds` column via migration; backend reads it into `ExecutionManifest.StallWallClockSeconds` and threads it into the deadline. Verified end-to-end (API update → read-back round-trip + UI renders the field). |
...
| Feature | **A canned `orchicon-dev` runtime image ships for dogfooding.** `:dev` (`deploy/runtime/Dockerfile.dev`) adds Go 1.26, Node 22, buf, atlas, and a baked PostgreSQL 15 so a worker can build and DB-test the Orchicon repo in-sandbox: it boots a disposable non-root Postgres (`initdb`/`pg_ctl` into the ephemeral tmpfs) and runs the full `make ci` DB path (migrations, RLS gate, DB unit tests) against `localhost:5432` — the instance dies with the container and never touches the plane's Postgres. Runtime containers mount the project's `project_dir` automatically at create time, so dogfooding needs no `sync-mounts` (that script applies only to the long-lived single-container instance). |
