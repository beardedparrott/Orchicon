...
| Chore | **Container image + Go cache hygiene.** Repeated builds orphaned ~229 dangling images (~240 GB) and the Go build cache grew to 35 GB. `scripts/container.sh build` and the runtime daemon's custom-image build now prune dangling images after each build; the runtime daemon also sweeps crash-leftover `orchicon-runtime-*` containers immediately on start (was: first sweep only after the interval). New `make clean` target clears the Go build cache + `bin/`. |
