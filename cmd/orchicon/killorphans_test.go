...
// reparented to init (PID 1) — exactly the shape a crashed plane leaves.
func startOrphanedSleep(t *testing.T) int {
	...
	out, err := exec.Command("/bin/sh", "-c", "/bin/sleep 300 >/dev/null 2>&1 & echo $!").Output()
	if err != nil {
		t.Fatalf("spawn a reparented sleep: %v", err)
	}
	pid, err := strconv.Atoi(strings.TrimSpace(string(out)))
	if err != nil {
		t.Fatalf("parse the orphan pid from %q: %v", out, err)
	}
	...
	t.Fatalf("pid %d was never reparented to init; the orphan contract cannot be tested here", pid)
	return 0
}

// ppidOf reads a process's parent from /proc directly — deliberately NOT via
// initParented, so a broken initParented cannot make these tests pass.
...
// running: it has been terminated and merely not reaped (this container's init
// does not reap, so a killed orphan lingers as one) — termination is what the
// sweep promises either way.
...
func TestKillOrphansReapsInitParentedAndSparesLiveParent(t *testing.T) {
	marker := fakePgrep(t)
...
// initParented is the whole safety property of the sweep, so it is asserted
...
func TestInitParented(t *testing.T) {
	live := startLiveParentedSleep(t)
	if initParented(live.Process.Pid) {
		t.Errorf("pid %d is this test's own child (parent %d), but initParented called it an init-parented orphan", live.Process.Pid, os.Getpid())
	}
	orphan := startOrphanedSleep(t)
	if !initParented(orphan) {
		t.Errorf("pid %d was reparented to init, but initParented called it not an orphan", orphan)
	}
	if initParented(1) {
		t.Error("PID 1 reported as an orphan: its parent is 0, and init is never the sweep's to kill")
	}
	if initParented(1 << 30) {
