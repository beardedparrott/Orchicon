...
- The Orchicon MCP is registered in `opencode.jsonc` as the `orchicon` server (`docker exec -i orchicon-cnt-prod orchicon mcp`, tenant `tnt_dev`), exposing the Ask Orchicon tool registry (`orchicon_*` tools) to every session in this repo. Dogfooding data lives on the **prod** instance, so that's the default; `orchicon-dev` is the same server against the dev container (keep exactly one `enabled: true` — both share tool names). The `orchicon_*` tools hit the instance's Postgres directly (same surface as `orchicon mcp`), so mutating tools create real data in that instance — verify the tenant/instance before using them.
