...
- **Workflow runtime containers** (pure per-workflow execution): one short-lived container per active workflow run, created when a run leaves `pending` and killed at a terminal state. `orchicon runtime-daemon` (host process owning the Docker socket; narrow unix-socket API, image/mount/argv allowlists) creates/execs/kills `orchicon-runtime-<runID>`; `orchicon runtime-supervisor` is the container PID 1 that streams `opencode run`; `orchicon runtime-client` forwards dispatches via `docker exec`. Security: the runtime runs as the host uid with the rootfs chowned to it (no root process), project dirs are written as the host user, host opencode data is mounted read-only with worker state redirected to an ephemeral XDG dir, and apt packages are build-time only (dpkg refuses non-root) — runtime installs use pip/npm/mise/uv into the ephemeral FS. Lifecycle hooks live in the WorkflowReconciler plus a 30s orphan-reap sweep in `server.go`; headless `orchicon serve` (no daemon socket) degrades to in-process execution. The execution guard lives in `internal/guard` (leaf package) and is built in-container by the supervisor. See DOCUMENTATION.md §Workflow Runtime Containers.
