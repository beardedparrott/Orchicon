...
- **Never re-verify an established fact.** If a fact is already recorded in the run's facts ledger (`.orchicon/<run_id>/facts_learned` or the composite prompt's "Facts learned (this run)" section), treat it as established — do not re-derive or re-confirm it. Re-confirming the same environmental condition multiple times within one session is the single largest source of wasted review/QA time (observed: a PR review re-validated the same three pre-existing conditions 3–4× each; a QA run burned ~10 turns re-deriving a root cause the implementer had already documented).
- **Append to the facts ledger.** When you establish a fact, root cause, environment gotcha, or decision that later steps should not re-derive, record it as a `FACTS LEARNED:` line in your final summary so it propagates to every downstream step. See §Worker environment baseline for the conditions already established — do not re-discover them.
- **Diminishing returns apply to review.** Reviewers and approvers: once you have verified the change against its acceptance criteria and found no blockers, stop. Do not keep opening deeper rabbit holes or re-running checks whose answers you already have. An empty findings list on a good change is a good result (see PR Reviewer `Behavior`).
...
## Worker environment baseline (known pre-existing conditions)

These conditions are **established facts** about this environment. Do not spend a session re-verifying them; when a check fails or an environment behaves oddly, compare against this baseline FIRST and record any new finding as a `FACTS LEARNED:` line.

- **`/tmp` is noexec in the runtime containers (and some hosts).** Go's default `TMPDIR` on noexec mounts makes guard tests (and any test binary that execs) fail with an exec-format/permission error. This is a pre-existing environmental condition, **not a regression**. Run tests with `GOTMPDIR=/tmp` (or `TMPDIR=...` on an exec-able mount) and treat guard failures under noexec `/tmp` as pre-existing — compare against the base, don't chase them.
- **Pre-existing gofmt drift exists at base.** Several files carry formatting drift that predates any current change. Only **new** formatting violations introduced by this change matter. Verify against the base (`git stash` or `git diff origin/develop`) once; do not fix unrelated files.
- **Whole-repo semgrep reports many pre-existing findings** (the deny-lists in existing code, ~181 hits across the repo, ~54 in `internal/opencode/config.go` alone). Scope semgrep to the files this change touched and report only genuine new findings:
  ```bash
  semgrep scan --config .orchicon/semgrep_orchicon.yml --error $(git diff --name-only origin/develop...HEAD | grep -E '\.go$')
  ```
- **The runtime-container supervisor runs the daemon self-copy present at container start.** In `:orchicon-dev` runtime containers, PID 1 is `orchicon runtime-supervisor` hosting the container's opencode serve. The supervisor binary is the version mounted at container create, so a container started before a feature landed runs the **pre-feature** daemon — the sandbox plane will not auto-boot there until the daemon rebuilds/restarts. If the sandbox plane is down in a container, check the running binary version first (`orchicon --version`), then rebuild/restart — do not re-investigate the whole boot path.
- **Sandbox plane (`:orchicon-dev` runtime images).** The disposable control plane inside the container runs at `http://localhost:8080` with Postgres on `localhost:5432` (user `orchicon`) and `ORCHICON_TELEMETRY=none`. `/healthz` mounts only after migrations, so a half-initialized plane cannot pass the run-start gate. Base/`:gui` images boot no plane.
...
5. Before the final commit on your branch, update `UPDATES.md` (new table row in the typed `| # | Type | Phase | Summary |` format) with a one-line summary describing the changes in this PR. Do **not** touch README.md's "Last Release Changes" section manually — `scripts/gen-release-notes.sh --sync-readme` keeps it in sync from `UPDATES.md`. This is the commit that will be PR'd and merged into `develop`.
...
- **Policy**: Rego (Open Policy Agent)
 - **Runtime adapters**: gRPC sidecars (OpenCode first, CLI now / IPC later). **Adapters are mounted, never baked**: Orchicon ships NO adapter CLI in its container images — the operator installs opencode on the host and `container.sh`/`orchicon install`/the runtime daemon bind-mount `~/.opencode` (ro) into the main and runtime containers, with the bin dir put on PATH. The supervisor's `runtimeBinAllowlist` (`internal/runtime/agent.go`) lists exec-able adapter binaries (`opencode` today; add `claude`/`codex` when those adapters land). Licensing-safe pattern for all future adapters: the product mounts the operator's own install, never redistributes the CLI. **Session transport is the adapter contract**: worker executions run as persistent opencode sessions (`internal/opencode/session.go` — create / `prompt_async` / abort / SSE), not one-shot `run` subprocesses; claude (`--local-sdk` + Agent SDK + `--btw` injection) and codex (TS Agent SDK) both support the same session/injection model, so `SessionClient` generalizes. Host serve (`internal/opencode/servehost.go`, always-on + supervised) serves the in-process population; workflow-run container serves come from the **warm pool** (see §Things you need to know → Workflow runtime containers) and are reached directly on the docker bridge (`CreateResponse.ServeURL = http://<container-ip>:4096` — never a published port: a containerized plane can't reach host loopback and docker-proxy races a lazily-starting serve). The container serve config omits the operator's MCP servers (`SkipUserMCP`) — a serve eagerly connects to every MCP at startup and the `orchicon` docker-exec entries hang it.
...
- The Orchicon MCP is registered in `opencode.jsonc` as the `orchicon` server (`docker exec -i orchicon-cnt-dev orchicon mcp`, tenant `tnt_dev`), exposing the Ask Orchicon tool registry (`orchicon_*` tools) to every session in this repo. **The DEV instance is the default and the ONLY instance for development work** — `orchicon-cnt-dev` on :8080/:3002. `orchicon-prod` (`orchicon-cnt-prod`, :8091/:3003) is registered but DISABLED (keep exactly one `enabled: true` — both share tool names). **Never create, mutate, or delete data on the PROD instance during development** — no work items, workers, workflows, runs, policies, or recovery actions. The `orchicon_*` tools hit the instance's Postgres directly (same surface as `orchicon mcp`), so mutating tools create real data in that instance; verify you are on `orchicon-cnt-dev` before using any mutating tool. Prod is the human's dogfooding/verification surface only; a session must never touch it without explicit, per-action human instruction.
...
- The `site/` landing page and `README.md` document the `orchicon` commands and installed files. Keep both in sync when commands, flags, or install paths change — on `develop` they are staged, never published. **Public deploy is main-gated:** CloudFlare Pages is pinned to `main` (Branch control / Production branch in the dashboard — see `CLOUDFLARE_SETUP.md`; never "Default branch", or every develop merge would publish unreleased landing-page content), so `site/` + `scripts/install.{sh,ps1}` only reach `orchicon.dev` when the human merges `develop` → `main`. `README.md` is not part of the site bundle at all — it ships only inside the main-gated GitHub Release asset. Update these files freely on feature branches; the human's release merge is the only thing that publishes them.
...
- **README.md** — update the Installation section if the commands or prerequisites change. The "Last Release Changes" section is auto-synced from `UPDATES.md` (`scripts/gen-release-notes.sh --sync-readme`) and the release body is generated the same way at release time; per-PR workers never touch it manually.
...
- **Landing page + install deploy**: `site/` holds the static landing page deployed to CloudFlare Pages (`orchicon-site`). The build step copies `scripts/install.{sh,ps1}` into the deployed bundle so the one-liner install commands work. `site/install` and `site/install.ps1` are git-ignored build artifacts. **Main-gated**: CloudFlare Pages is pinned to `main` (Production branch — see `CLOUDFLARE_SETUP.md`), so `site/` and the install scripts only publish when the human merges `develop` → `main`; on `develop` they are staged. Full setup in `CLOUDFLARE_SETUP.md`.
...
- **Workflow runtime containers** (pure per-workflow execution) run on a **warm pool** (`internal/runtime/pool.go`): containers are keyed by environment (image + project mounts), **leased exclusively per run** at run start, and **reset to pristine in the background** on release (rm -f + recreate with the identical spec + warm the serve), so dispatch never cold-starts a container or serve and nothing from one run's state (installed packages, `/tmp`, opencode sessions/data) crosses to the next. `orchicon runtime-daemon` (host process owning the Docker socket; narrow unix-socket API, image/mount allowlists) leases/resets pool containers (`POST`/`DELETE /v1/runtimes`) and builds/removes custom runtime images (`GET/POST/DELETE /v1/images`); `orchicon runtime-supervisor` is the container PID 1 that hosts the container's **opencode serve** (detached child, serve handshake + ping only — the one-shot exec/stream/reconnect machinery was removed); `orchicon runtime-client` forwards the daemon's serve handshake / ping via `docker exec`. **Run-start serve gate**: a workflow run persists `runtime_ready` (forward-only migration); the pending→running transition sets it false, an async ensure-serving pass (one goroutine per run, idempotent) leases the container, warms + **proves the serve usable** (L1: `/global/health` + a real session-create round-trip) within `ORCHICON_RUNTIME_SERVE_READY_TIMEOUT` (default 120s) and flips it true, and DAG step progression is HELD while it is false — no execution ever starts against a cold serve; a serve that can't come up **fails the run at start** instead of looping step recovery. Security: the runtime runs as the host uid with the rootfs chowned to it (no root process), project dirs are written as the host user, host opencode data is mounted read-only with worker state redirected to an ephemeral XDG dir, and apt packages are build-time only (dpkg refuses non-root) — runtime installs use pip/npm/mise/uv into the ephemeral FS. **Runtime images** (`internal/runtimeimage/` + the Runtime Images UI page) are per-work-item buildable containers: the daemon always rewrites `FROM <base>` and injects the `org.orchicon.runtime-base=true` label (base is always part of every image; that label is also the container-create gate). The orchicon binary is **not baked** into any runtime image — the daemon **self-copies its executable to a stable path next to the socket at startup** (`cmd/orchicon/runtime.go` `copySelf`) and bind-mounts the copy read-only at `/usr/local/bin/orchicon` on container create, so a rebuilt binary needs no image rebuild AND `make clean` (which deletes `bin/orchicon`) can never orphan the mount. Stock images (`base`/`:gui`/`:dev`) are version-gated by an `org.orchicon.runtime.version` label (app version + Dockerfile SHA-256; `:gui`/`:dev` embed the base's version for the cascade) so `container-build` skips them when nothing changed (and skips any image carrying `org.orchicon.runtime.spec-version` — a tenant-owned daemon build from an edited canned row — so container.sh never clobbers it); custom-image Deploy short-circuits when `built_version == version` on the row. **Canned stock images**: `runtimeimage.SeedCannedImages` (called at boot, mirrors `SeedDevWorkers`) seeds one editable `runtime_images` row (`source='stock'`) per daemon-reported stock variant, with the shipped Dockerfile template + a self-consistent `# orchicon.seed=<sha12>` marker as the override — current seeds reconcile in place, stale seeds roll forward (version bumps → async auto-build, which prunes the previous tag version), and user-edited rows are never touched; deleted canned rows re-seed next boot. The base + `:gui` (headless GUI libs) + `:dev` (Go/Node/buf/atlas + baked Postgres for dogfooding Orchicon) variants ship on GHCR; `ORCHICON_RUNTIME_IMAGES` adds stock allowlist entries. Runtime containers mount the project's `project_dir` **automatically** at lease time (the lifecycle reads it from the projects table — `sync-mounts` only applies to the long-lived single-container instance). Pool lifecycle: the daemon resets all containers at start (covering plane-down leaks) and idle-reaps clean containers (`ORCHICON_RUNTIME_POOL_CAP` default 1 per env, `ORCHICON_RUNTIME_POOL_IDLE` default 10m); a boot adopt pass ensures leases for active runs. Headless `orchicon serve` (no daemon socket) degrades to in-process execution. The execution guard lives in `internal/guard` (leaf package) and is built in-container by the supervisor. The runtime image ships to GHCR (`ghcr.io/beardedparrott/orchicon-runtime`) and `orchicon install` (invoked by the one-line installers) pulls it, starts the daemon, and launches the container. **Sandbox plane on `:orchicon-dev`**: the supervisor boots a FULL, disposable Orchicon control plane inside the container at start — Postgres (`localhost:5432`, user `orchicon`) → NATS → `orchicon serve` on `http://localhost:8080` — so workers on the dev image get a consistent environment: `curl http://localhost:8080/healthz`, DB-backed tests against `localhost:5432`, and the `orchicon_*` MCP tools against the **sandbox DB** (the run-start gate verifies the plane's `/healthz` before any execution dispatches). **The sandbox plane is YOURS and only yours — never the host plane**: it dies with the container (pool reset recreates pristine) and never touches the host plane's Postgres; do not use it to reach any host-plane service or DB, and never run a second `orchicon serve` inside the container. Base/`:gui` images boot no plane (no MCP) — identical to today. See DOCUMENTATION.md §Workflow Runtime Containers → Sandbox plane.
...
> Read this before starting any work. All changes must be recorded in `UPDATES.md` in the typed table format below.

Phase progress and per-PR changes are tracked in `UPDATES.md`. Before commencing any work, always read `UPDATES.md` first to understand the current state and what has already been shipped.

**Format** — one row per PR, appended to the TOP (newest first). Row numbers are **monotonic** — never renumber: the release-notes generator derives the "since last release" boundary from the previous release's copy of `UPDATES.md` (in git), so renumbering would corrupt it. The table is:

```
| # (next number) | Type | Phase | One-line summary |
```

Type ∈ `Feature | Bug fix | Chore | Docs | Refactor | Test`. Phase is a short area label (e.g. `Sequence engine`, `Session transport`, `Auth`, `UI`). Keep the summary to ONE line — the verbose detail lives in the PR description, not the file.

**Lifecycle (release-notes automation):**
- **On develop** — append your row. That is the ONLY UPDATES.md change a per-PR worker ever makes. README.md's "Last Release Changes" stays in sync automatically via `scripts/gen-release-notes.sh --sync-readme` — one source of truth (UPDATES.md), two consumers (README + the release body). On develop this is staged, never published (CF Pages + release assets are main-gated).
- **At release** — `release.yml` runs `scripts/gen-release-notes.sh` to generate the GitHub Release body from every row accumulated since the previous release, grouped by Type → Phase. README already carries the same block; no human release-time write is needed.
- **After a release, on the next develop merge** — run `scripts/gen-release-notes.sh --trim` to drop the released rows (keeping only the current cycle) and re-sync README in the same step. It refuses to trim when it can't determine the previous release, and is a no-op when there's nothing to trim. This keeps both files small forever; released rows stay preserved in the git tag + the release body.

If no existing phase fits, group related PRs under a descriptive phase name. Keep entries concise — one line per PR.
