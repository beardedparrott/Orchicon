...
| 10 | Project context files | done | Projects now support `project_dir` (root directory) + `context_files` (relative file paths selected as context). New `FileBrowser` component on project detail page with file-tree view, multi-select checkboxes, and Select All/Deselect All. `ListProjectFiles` RPC returns recursive file tree. Selected files' contents are read and injected into `buildCompositePrompt` as a new "# File context" section. Migration adds `project_dir` text + `context_files` jsonb columns to projects table. Files larger than 1 MiB are skipped. `make ci` passes. |
...
- **Three execution-flow fixes** (branch `fix/execution-errors-and-nav`): (a) `opencode.Adapter.parseStdoutLine` was reading `evt["message"]` for the `type:"error"` event when the actual message lives at `evt["error"]["data"]["message"]`; the parsed message is now stashed in `lastStreamErr` and folded into `OnResult` so `error_message` carries the real provider reason (rate-limit, 500, etc.) instead of just "exit status 1". (b) `db.CreateRecoveryExecution` had a skipped `$12` in the parameter list so Postgres rejected the insert with SQLSTATE 42P18 on every failure — `TriggerRecovery` had been silently failing all day; renumbered contiguously. (c) The workflow run view file `workflows_.$id.runs.$runId.tsx` was nested under the editor in the generated route tree (dot is the path delimiter), but the editor does not render `<Outlet/>`; renamed to `workflows_.$id_.runs.$runId.tsx` so the `runs` segment escapes the `$id` nest and the route becomes a top-level sibling — the run view now actually renders.
