...
3. Each card links to the work item (`/work-items/$id`) and its bound workflow (`/workflows/$id`); a right-aligned frequency slot shows **One-time** for single-run items and the recurrence description (e.g. "daily", "every 2 days") for recurring items, which also carry a fuchsia **Recurring** badge next to their kind badge so recurring schedules are visibly distinct
