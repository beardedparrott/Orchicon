...
**The orchicon binary is mounted, never baked:** the runtime images contain **no `orchicon` binary** — the daemon bind-mounts **its own executable** (`os.Executable()`, resolved at process start) read-only at `/usr/local/bin/orchicon` in every runtime container it creates. Docker resolves bind sources at `docker run` time, so a rebuilt `bin/orchicon` is picked up by newly-created containers with **no image rebuild** and no daemon restart; already-running per-workflow containers keep their inode until reaped (they are recreated per run anyway). The mount target sits on the baked PATH, so both the `orchicon runtime-supervisor` entrypoint and the `orchicon runtime-client` `docker exec` paths resolve. If the executable is missing the daemon skips the mount (logged warning) rather than failing `docker run`. This is the same "mount, never bake" pattern as the adapter CLIs below.
