...
3. **Defaults → Default models**:
 ...
 5. **Defaults → Execution budget (defaults)**: default per-execution budget ceilings (`tokens`, `cost_usd`, `wall_clock_seconds`, `tool_call_count`) applied when a worker does not set its own value for a field. **A worker's own `budget_overrides` always overrides these per-field**; empty tenant fields fall back to the built-in defaults (tokens 1,000,000, cost $10, wall clock 3600s, tool calls 100). The wall clock is the hard per-execution deadline that kills the subprocess even if the model is still producing output (the runaway-spend backstop); an explicit `wall_clock_seconds: 0` on a worker disables that worker's timeout. Env override: `ORCHICON_STALL_WALL_CLOCK_SECONDS`. **`cost_usd` / `tokens` are the SOFT-FIRST compaction trigger** (not a hard abort): when the accumulated cache-aware spend breaches the configured budget at a quiet step boundary, the adapter asks opencode to compact the session (`POST /session/:id/summarize`) once and **continues** — wall-clock and stall detection remain the only live killers. Cost is cache-aware (the primary gate sums the runtime's provider-discounted per-step `part.cost`); the `tokens` fallback (used when no `cost_usd` is set) counts fresh input+output+reasoning plus `cache.read` weighted by `ORCHICON_COMPACT_CACHE_DISCOUNT`, so cache is never excluded from the estimate but cheap-cache providers don't false-fire. See `architecture-notes/compact-session-on-token-budget-breach.md`.
...
| `ORCHICON_COMPACT_CACHE_DISCOUNT` | `0.1` | Cache-read weight in the compact token-fallback gate. Only used when no `cost_usd` budget is set. Lower = cheap-cache providers (DeepSeek ≈ 5%) don't false-fire on cache bloat; raise toward ~0.3 for paid-cache providers (Claude) |
