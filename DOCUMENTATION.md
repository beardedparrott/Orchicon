...
2. The **Upcoming** view (default) lists scheduled work items (`status = scheduled`) in chronological order with their next runtimes, grouped by local day (Today / Tomorrow / weekday date). Below the scheduled agenda, a **Queued** section lists the not-yet-armed children of a running sequence parent (pending items derived from the full project list — sequence children carry no `scheduled_start_at` of their own, only the parent is scheduled), ordered by chain order
