...
| `orchicon run-context <runID>` | Print a workflow run's `.orchicon` archive (per-step files + run summary/status) for context-by-reference — the companion to the delta handoff. Flags: `--grep <pattern>`, `--steps|--summary`, `--limit N`. Pull the full detail of an earlier step without re-embedding it into the prompt. |
...
Every dispatch prompt is prefixed with a **stable prompt prefix** (`db.StablePromptPrefix` in `internal/db/prompt.go`) built ONLY from shared constants: the fixed **worker identity preamble** — *"You are an autonomous worker running inside the Orchicon orchestration platform. You are not a human operator and there is no human attached to this run. You execute one assigned work item per run, operate within your role and the project's acceptance criteria, and report your result via the `ORCHICON WORKER SUMMARY` contract. Work autonomously to completion; do not wait for interactive approval for work that is within your assigned scope."* — followed by the shared **safety rules** (`## Safety rules (HARD limits)`), the **efficiency directives** (`## Efficiency — minimize tool output and tool calls`: compact git/gh output + batch-tool-calls guidance for local-model token overhead), and the machine-generated **`## Runtime environment`** block. Because the prefix is identical across every worker and step of a run, llama.cpp's prefix-based KV/prompt cache reuses it across all steps and roles instead of re-prefilling per step. Everything role/step-specific — worker identity (Role, Skills, Behavior, AGENTS.md), the task, project/work-item context, instructions, and execution history — follows AFTER the prefix. The canned workers also carry the same identity sentence at the start of their seeded Role (`cannedWorkerIdentity` in `internal/db/seed_workers.go`), so the stored worker row and the dispatched prompt agree. This is the mechanism that distinguishes an **in-Orchicon worker** (autonomous: PRs into `develop`, merges on approval, reports via the summary contract) from a **human-facing session** (an agent in a chat that must ask before PRing/merging). The safety rules ship via the prefix only — the seeder strips them from the canned workers' stored AGENTS.md (replaced by the versioned seed marker) so they are not duplicated per worker; workers additionally receive a soft + hard cap on summary length (`capSummaryNarrative`, ~500 tokens) because the summary is re-embedded into every later step's prompt.
...
Dependency edges form a **DAG** (`depends_on`, `blocks`); `relates_to` is a symmetric, non-ordering relationship and is exempt from the DAG invariant (a `relates_to` pair is never a cycle). Cycle enforcement is **defense in depth**: the app layer rejects a cycle on `AddDependency`/`UpdateWorkItem` set-replace with a `FailedPrecondition` naming the offending edge (`cannot add dependency X -> Y: would create a cycle in the work DAG`), and a **DB trigger** (`enforce_work_dag_acyclic`, `BEFORE INSERT OR UPDATE` on `work_item_dependencies`) enforces the same invariant at the persistence boundary — so invalid graphs can never persist through any writer, including a future bulk-import path or raw SQL. The trigger rejects self-loops on every edge type, exempts `relates_to` from the reachability check, and serializes concurrent DAG-edge writers per project with a transactional advisory lock so the concurrent-write TOCTOU race (two inserts `A→B` + `B→A` both passing under READ COMMITTED) cannot persist a live cycle — the second writer re-walks after the first commits and is rejected.
...
- **Board** — a Jira-style kanban with one column per server status (Pending/Ready/Assigned/Running/Succeeded/Failed/Cancelled; checkpointing/recovering render in Running, scheduled/recurring/blocked in Pending — blocked cards keep their distinct teal pill). Cards drag & drop between columns via dnd-kit (`@dnd-kit/core` + `sortable` + `utilities`); drops are **server-confirmed** (no optimistic transitions) with a transient "moving…" state and toasts. Two advisory gates reject obviously-wrong moves before the mutation: a **blocked** item cannot be dropped on Ready, and Epics/Features accept only pending/succeeded/cancelled. A per-card **"Move to…"** menu performs the identical mutation for keyboard/touch users. A blocked item (system-managed, like running/checkpointing/recovering) is not manually movable — the reconcilers own it and clear it automatically.
...
**Blocked (waiting) status:** when an item is armed/on-deck but cannot dispatch because an upstream `depends_on`/`blocks` edge is not terminal-success, the reconcilers surface it as **`blocked`** — a persistent, system-managed status (teal pill, distinct from `pending`/`scheduled`), so operators understand why nothing is dispatching instead of seeing a silent gray pill. Both silent-park sites flip to it: a ready standalone task with unsatisfied deps (the TaskReconciler parks `ready` → `blocked` on the dependency gate) and an on-deck sequence child (the sequence engine parks `pending` → `blocked` on the gate). Clearing is automatic — the next reconcile pass re-evaluates the gate and flips `blocked` → `ready` (standalone, dispatching in the same pass) or `blocked` → `pending` → arm (sequence); the TaskReconciler scan picks up `blocked` tasks (`ListBlockedTasks`) alongside ready ones. Only terminal-**success** unblocks (a failed/cancelled blocker leaves the dependent `blocked`, named and visible for the operator to resolve). No dispatch ever happens from `blocked` — dispatch proceeds only from `ready` (after a satisfied gate) or `pending` (sequence arming after a satisfied gate). The server names the blocking edges on every read (`WorkItem.blocked_by`, computed at read time from the DAG + statuses — never stored, so it cannot go stale); the tree/board render a "Blocked by: …" chip and the detail page lists the blockers. Strict-sequence children waiting on an in-order predecessor are NOT marked blocked (normal chain progression, shown by the `#N` badge). `blocked` is system-managed: user/operator input rejects it, it is not manually movable on the board, and kind switches / stops / resets route it to `pending` like any other non-in-flight status.
...
4. The **Running** view (toggle or `?view=running`) shows **any currently running workflow**: work items whose bound workflow run is in flight (status `RUNNING` / `CHECKPOINTING` / `RECOVERING` with a `workflow_run_id`), whether or not they carried a scheduled start — a workflow started manually, or via "Start immediately on save", shows here too. **Sequence parents** (active items with children and no bound workflow) also appear, labeled with a color-coded **multi-workflow** chip; their children carry the same chip plus a `#N` chain-order badge. The shown start time falls back to the item's `updated_at`/`created_at` when there is no schedule
 5. The **History** view (toggle or `?view=history`) is **run-driven**: it enumerates the tenant's `workflow_runs` for the selected scope (project, or all projects) via `ListWorkflowRuns` (scoped by `project_id`, sorted by each run's real `started_at`), rendering **one card per executed run**, most recent first, with a link to the run. Because history is keyed on the run record rather than the work item's status, it includes recurring fires whose item re-armed to `scheduled`/`recurring`, **every** prior run of a work item that ran more than once (not just its latest), and in-flight runs — a run only appears once it has actually started (`started_at` set). Cards link to the run (`/workflows/$id/runs/$runId`); when the bound work item still exists its title/kind are shown, otherwise the card falls back to the workflow name. Ordering uses the run's real `started_at` (descending by default), never an item's next/future scheduled firing time
...
- **Retry auto-resumes**: fix + retry the failed child to success and — because the cursor is derived — the chain continues with the next sibling automatically (the parent revives to `running` when its children are no longer halted).
- **Dependency gate / blocked**: an on-deck child whose external blockers aren't satisfied is **surfaced as `blocked`** (a persistent, system-managed status set by the engine — teal pill in the UI, distinct from `pending`/`scheduled`) instead of silently parked as `pending`. The parent stays `running` and the child stays `blocked`, and the engine re-evaluates the gate on every pass: when the blockers become terminal-success it flips the child `blocked` → `pending` and arms it in the same pass — clearing is automatic, no human action. Only a human may fail a blocked child (a failed/cancelled blocker leaves the dependent `blocked`, named and visible, so an operator can decide how to resolve it). The server names the blocking edges on every read (`WorkItem.blocked_by`, computed at read time from the DAG + statuses) and the UI renders a "Blocked by: …" tooltip + a detail-page blockers list. Strict-sequence children waiting on an in-order predecessor are **not** marked blocked (normal chain progression, communicated by the `#N` position badge).
...
A run carries a facts ledger so later steps inherit established facts instead of re-deriving them. Each worker records facts it established (root causes, environment gotchas, decisions) as `FACTS LEARNED:` lines inside its `ORCHICON WORKER SUMMARY`. The **single authoritative source** is the `.orchicon/<run_id>/facts_learned` file: the TaskReconciler appends every step's `FACTS LEARNED:` lines to it as the step completes, each line carrying the originating step name (`FACTS LEARNED (from <step-name>): <fact>`) so per-step attribution survives. The composite prompt instructs workers to read `.orchicon/<run_id>/facts_learned` first and no longer embeds an aggregate `## Facts learned (this run)` block — facts reach downstream workers from exactly one source. A fact already recorded is treated as established — workers are told not to re-verify or re-derive it, and to append a correcting `FACTS LEARNED:` line rather than re-investigate. This directly counters the observed over-verification pattern in review/QA steps (re-confirming the same environmental conditions 3–4× per session).

**Facts are scoped to a single run:**
The facts ledger above is scoped to a single run — and per-run `.orchicon/` state lives in a worktree that is pruned when the run ends. There is deliberately **no cross-fire carry-forward**: an early experiment rendered a `## Carry-forward — facts learned by the previous fire` block from the item's acceptance review into every composite, but it is unbounded by design — every fire's facts echo into all future fires forever, so context grows monotonically with fire count. The within-run ledger stays (facts flow from step to step inside a run); across fires, lifecycle state is the ledger: the Idea Cloud's **Rejected** section (below) is the durable, bounded queryable memory for what previous fires proposed and a human rejected, while accepted history lives in the normal work items. No prompt block compounds with recurrence.
...
Git-backed runs are dispatched with their working directory set to an **isolated worktree** at `<project_dir>/.orchicon-worktrees/<runID>/`, created by the `WorktreeReconciler` against a per-run branch off `develop` (providing a base ref). A **retry of a failed run re-attaches to the same branch** — the previous attempt's commits carry over (never `-b` a new branch); the branch is deleted **only on run SUCCESS**, gated on it being merged into the base (and never the current branch / `main` / `develop`; `origin develop` is fetched before the merge gate so a `gh pr merge` that advanced only the remote is not mistaken for unmerged). Because the reclaim is coupled to the prune pass (which only reaps a **`ready`** worktree), the reconciler scan additionally runs an **orphan-branch sweep** (`sweepOrphanBranches`): any **completed** run (or step run of a completed run) whose worktree is already `pruned` but that still records `worktree_branch` has that branch re-checked against the same proof-deletion gate and removed if it still exists locally, has no live worktree, and is provably merged — reclaiming refs that escaped the prune-time delete (e.g. the run completed after its worktree was taken down by an earlier pass). Success-only holds for the sweep: a failed/aborted run's branch is never swept. Non-repo projects fall back to in-place runs in `project_dir`. **PR links**: the per-branch worker reports `PR_URL:` / `PR_STATE:` lines in its summary (the last occurrence wins), which the scheduler captures into the run's `run_context` JSONB and mirrors onto the `WorkflowRun` / `WorkerExecution` rows — the board/list/run/execution views render a PR chip only for completed runs with a real captured URL (no compare-page fallback). For runs that completed before capture, `orchicon backfill-pr` idempotently resolves each git-backed terminal run's branch to its real PR via `gh --state all` and writes `pr_url`/`pr_state` into both the run context and the execution columns; no-PR runs stay linkless.

**Git strategy contract (`git_strategy`):**
A run's effective git strategy is resolved once, at every layer, from the same single source of truth (`db.EffectiveGitStrategy`): **workflow-level wins, else project-level, else `"local"`**. It is honoured end-to-end — stored, resolved, and enforced — rather than only consulted, so a strategy change never leaks branches. The three strategies guarantee:

- **`local`** — the default. The run worktree creates a named branch off `develop`; the worker commits and pushes to origin; the branch is reclaimed on run success via the proof-deletion gate (`branchProvablyMerged` + P1/P2/P3 + Gate B). A `none`-agnostic SDLC title can't accidentally skip the PR gate, and the runtime container always receives a push-capable `GH_TOKEN` and the git credential mounts.
- **`pr`** — identical run-worktree/branch provisioning to `local`, plus the `requires_pr` step gate: the PR is required before the branch is reclaimed. The runtime container carries a push-capable `GH_TOKEN`; PR-path step containers are never disturbed.
- **`none`** — **ephemeral**. The run worktree is a **detached HEAD** (no named branch is created, so there is no ref that can leak locally or on origin); the runtime container is created **without** a push-capable `GH_TOKEN` and **without** the git credential mounts (read-only/scoped tokens are used only when a step needs `gh`/plane reads); the worker's seed prompts and the per-dispatch `GitGuidanceBlock` tell it explicitly not to create a branch, commit a branch, or push anything to origin. Because no branch is created, "no branch retained" is structural rather than a cleanup promise, and `none` never trips `requires_pr`.

**Remote half of the prune:** for `local`/`pr` runs, when a local branch deletion succeeds through the existing proof gate, the reconciler also issues `git push origin --delete <branch>`, fail-closed: the remote delete runs **first**, and a remote-delete failure returns an error **without** deleting the local branch, so the run row keeps its `worktree_branch` provenance and the orphan sweep retries both. When origin is absent (probe inconclusive) the remote delete is skipped but a provably-merged local delete still proceeds. Nothing unmerged is ever deleted locally or remotely — P1/P2/P3 + Gate B are preserved.
...
Ask Orchicon runs on a **directory-less** session (the no-project execution guard blocks general-purpose file tools), so it needs a dedicated surface to see a project's files. Two **read-only** `orchicon_*` tools close that gap for **all three** conversation modes (registered in `allTools()`, so they appear in every mode's prompt and in the MCP server with zero per-mode plumbing):
...
- **Local dev**: The embedded OpenID Provider is the default IdP — no
  ...
  to `/login`.
- **Changing a local-account password** — Admin → Identities → the
  identity row-edit modal's Username + new-password fields (calls the
  admin-only `SetLocalCredential` RPC, `auth:write`; the plaintext is
