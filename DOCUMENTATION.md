...
- **`orchicon runtime-supervisor`** (PID 1 inside each runtime container): listens on a unix socket (`/tmp/orchicon-agent.sock`), streams exec output back, and signals children by exec_id. Hosts the container's **opencode serve** (detached child, `Cmd:"serve"`, 0.0.0.0:4096, guard + **stable** XDG data dir `/tmp/orchicon-serve-data` so sessions survive restarts), answers idempotent serve handshakes (owning the serve password, liveness-gated — a wedged serve is restarted, not reported as up), and runs a **serve watchdog** that polls `/global/health` and restarts the serve in place (same port + password) when it stops answering. Builds the execution-guard shim in-container so workers run under the same `rm`/`sudo`/`dd`/`mkfs` path-scoped safety guard as the in-process path. (The legacy one-shot `opencode run` exec path was removed with the transport it served.)
...
- **Fail-fast dispatch**: `sessionClientFor` returning nil (no host serve, or the daemon's `startServe` failing to bring the container serve up within 30s) is now a hard error → `failed_to_start` → workflow recovery. The daemon's `createRuntime` propagates serve-start errors instead of silently degrading.
...
| `ORCHICON_ASK_TIMEOUT` | `600s` | Ask Orchicon dispatch handshake bound: subscribe + send-accept + first event for a turn. A wedged serve fails fast instead of silently queuing |
