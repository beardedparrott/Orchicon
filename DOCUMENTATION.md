...
- **Filtering** — search/sort go server-side; kind + status compose client-side over the full fetched set (pageSize 1000) so the tree hierarchy stays intact. Select-all is tri-state over the visible filtered set and the selection clears on any filter change.
