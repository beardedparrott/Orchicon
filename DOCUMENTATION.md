...
**Cross-fire facts carry-forward:**
The facts ledger above is scoped to a single run — and per-run `.orchicon/` state lives in a worktree that is pruned when the run ends, so for a **recurring work item** the next fire starts with no memory of what the previous fire established. To close that gap, both composite builders (workflow `buildCompositePrompt` and standalone `buildStandaloneComposite`) render a **carry-forward block (`## Carry-forward — facts learned by the previous fire`)** from the item's `AcceptanceReview` — which the WorkflowReconciler overwrites with the previous fire's acceptance review on every fire, so at fire time it holds fire N−1. The block extracts every `FACTS LEARNED:` / `FACTS LEARNED (from <step>):` line (skipping the instructional contract lines) plus the fire's delivery summary, bounded to 8K chars, and renders **nothing** when the review is empty or absent — first fires, manually-run items, and one-shot tasks are byte-identical to before. The next fire therefore inherits the previous fire's corrections (source quirks, dedupe states, tooling gotchas) instead of re-deriving them; the correction protocol on facts is unchanged (append a correcting `FACTS LEARNED:` line, never silently re-derive).
