...
// locks for the visual Worker editor.
// All mutations write through the transactional outbox.
// MUTABILITY, as the code enforces it today:
//   - HEADER text (name/description/purpose) is writable on any worker that is not
//     retired, and the RBAC role binding is writable on every status. Neither is
//     versioned content, and nothing resolves a worker by name (steps reference it
//     by id; the slug is immutable after create). UpdateWorker in internal/db is
//     the gate.
//   - A PUBLISHED version has its fields edited IN PLACE without advancing the
//     version number: UpdateWorkerVersion{republish} reverts, updates and
//     republishes in one transaction, and BulkUpdateWorkerModel does the same for
//     model_ref.
//   - CreateWorkerVersion{publish} creates the next version and publishes it in
//     one transaction, so a failure cannot strand an unpublished draft.
...
type UpdateWorkerRequest struct {
	...
	// name/description/purpose are plain strings, so EMPTY MEANS UNCHANGED —
	// absence and "" are indistinguishable to the server. Clearing one is
	// therefore not expressible through this message today; expressing it needs
	// `optional` (presence) on these fields.
	Name        string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	...
	// Editable on every status, and may ride in the same request as header text.
